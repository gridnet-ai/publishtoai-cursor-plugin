...
function writeInitScaffold(): void {
  ...
  if (fs.existsSync(target)) {
    console.log(chalk.yellow('web4page.json already exists. Run `publishtoai validate` to check it.'));
    return;
  }
  const scaffold = {
  ...
  hint('  2. Run: publishtoai validate');
  hint('  3. Run: publishtoai login');
  hint('     Or: publishtoai publish (login runs automatically)');
}
...
async function runPublishAll(): Promise<void> {
  ...
  if (files.length === 0) die('No web4page.json files found. Run `publishtoai init` first.');
...
program
  .command('login')
  .description('Sign in via browser and save API key to ~/.publishtoai/config.json')
  .action(async () => {
    try {
